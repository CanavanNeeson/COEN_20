...
int main(void)
	...
	InitializeHardware(HEADER, "Lab 6a: Reversing Bits & Bytes") ;
	...
	while (1)
		...
		row = PutStringAt(row, LABEL_COL, "ReverseBits (%d clock cycles)",
			(stop - strt)/1000 - overhead) ;
		...
		row = PutStringAt(row, LABEL_COL, "ReverseBytes (%d clock cycles)",
			(stop - strt)/1000 - overhead) ;
		DrawHLine(LABEL_COL, row, XPIXELS - 2*LABEL_COL) ;
